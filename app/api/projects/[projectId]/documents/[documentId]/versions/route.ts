...
import { createVersionSchema } from "@/src/lib/validation";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const result = await parseJsonBody(request, createVersionSchema);
  ...
  const version = await prisma.$transaction(async (tx: TransactionClient) => {
